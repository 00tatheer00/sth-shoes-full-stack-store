...
export const productService = {
  ...
  async getProducts(params?: {
    ...
    try {
      ...
      console.error('Error in productService.getProducts:', e);
      return this.filterLocalProducts(params);
    }
  },
  ...
  async getProductBySlug(slug: string): Promise<Product | null> {
    try {
      if (process.env.NEXT_PUBLIC_SUPABASE_URL) {
        ...
        if (!error && data) {
          return {
            ...
            materials: ['100% Genuine Full-Grain Cowhide Leather', 'Recycled Tyre Rubber Sole'],
            craftingDetails: ['Handcrafted in Peshawar'],
      ...
      return dataEngine.getProductBySlug(slug) || dataEngine.getProducts()[0] || null;
    } catch (e) {
      return dataEngine.getProductBySlug(slug) || dataEngine.getProducts()[0] || null;
    }
  },
  ...
  filterLocalProducts(params?: any): Product[] {
    let result = dataEngine.getProducts();
