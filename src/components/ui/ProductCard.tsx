...
export const ProductCard: React.FC<ProductCardProps> = ({ product, showCountdown }) => {
  ...
  return (
    <div className="group bg-white border-2 border-[#DBEAFE] hover:border-[#1E90FF] card-dodger-hover flex flex-col justify-between overflow-hidden relative rounded-none shadow-xs">
      {/* Product Image Stage */}
      <div className="relative aspect-square bg-[#F4F8FF] overflow-hidden">
        ...
        <div className="absolute top-3 left-3 flex flex-col gap-1.5 z-10 pointer-events-none">
          ...
          {product.isNew && (
            <span className="badge-dodger-3d px-2.5 py-1 text-[10px] font-mono font-bold uppercase tracking-widest border border-white/40 shadow-md">
              NEW ARRIVAL
            </span>
          )}
        ...
        <button
          ...
          className={`absolute top-3 right-3 p-2.5 rounded-full transition-all duration-300 z-20 shadow-md ${
            isSaved
              ...
              : 'bg-white/90 text-[#0B1120] hover:bg-[#0B1120] hover:text-[#00E5FF]'
          }`}
        ...
        {/* Deal Countdown Bar */}
        {showCountdown && (
          <div className="absolute bottom-0 inset-x-0 bg-[#0B1120]/95 text-white py-1.5 px-2 text-[10px] font-mono text-center flex items-center justify-center gap-1.5 z-10 border-t border-[#00E5FF]/40">
        ...
        {!showCountdown && (
          <div className="absolute bottom-3 left-3 right-3 opacity-0 group-hover:opacity-100 translate-y-2 group-hover:translate-y-0 transition-all duration-300 flex gap-2 z-20">
            <button
              ...
              className="flex-1 py-2.5 bg-gradient-to-r from-[#1E90FF] to-[#0B63F6] text-white hover:from-[#00E5FF] hover:to-[#1E90FF] hover:text-[#0B1120] text-xs font-serif font-bold uppercase tracking-wider flex items-center justify-center gap-1.5 shadow-xl transition-colors border border-white/30"
            >
              {added ? (
                <>
            ...
            <Link
              ...
              className="p-2.5 bg-white text-[#0B1120] hover:bg-[#EFF6FF] shadow-xl flex items-center justify-center transition-colors border border-[#DBEAFE]"
              title="Quick View Details"
            >
              <Eye className="w-4 h-4 text-[#1E90FF]" />
            </Link>
          </div>
        )}
      </div>
      ...
      <div className="p-4 flex-1 flex flex-col justify-between bg-white border-t border-[#DBEAFE] space-y-3">
        <div>
          <div className="text-[10px] font-mono uppercase tracking-widest text-[#1E90FF] font-bold mb-1 flex justify-between items-center">
            <span>{product.category}</span>
            <span className="text-[9px] font-mono text-slate-400">EU 39-46</span>
          </div>

          <Link href={`/product/${product.slug}`} className="block group-hover:text-[#1E90FF] transition-colors">
            <h3 className="text-base font-serif font-bold text-[#0B1120] line-clamp-1">
              {product.name}
            </h3>
          </Link>

          <div className="flex items-center gap-1.5 mt-1 text-xs text-[#FFB800]">
            <div className="flex">
            ...
            <span className="text-[11px] font-mono font-bold text-[#0B1120]">{product.rating}</span>
        ...
        <div className="pt-2 border-t border-slate-100 flex items-center justify-between">
          <div className="flex items-baseline gap-2">
            <span className="text-lg font-serif font-bold text-[#0B1120]">
              ...
              <button
                ...
                className={`w-4 h-4 rounded-full border transition-all ${
                  selectedColor.name === color.name
                    ? 'border-[#1E90FF] scale-125 ring-2 ring-[#1E90FF]/40 shadow-xs'
